...
open class TaskGenIntelliJRuns : GroupedTask() {
    ...
    fun execute() {
        ...
        logger.info("run configuration detected: ${file.absolutePath}")
...
//         inject run configuration into config file
        ...
        document.injectRunConfiguration("RunFluidity (${getProjectName()})", "me.yuugiri.agent.AgentLoader", "-Djava.library.path=\"${nativesDir.canonicalPath}\" $vmParams",
            "$args $programParams", gameDir, version.getJars(cacheDir).toMutableList().apply { add(Jvm.current().toolsJar!!); add(mainJar) },
            listOf(minecraftJar(project)))
